...
|-------|---------------|-------|---------|
| Codebase Analysis | workflow:codebase-analyzer | sonnet | Extract conventions (always regenerate) |
| Planning | workflow:architect | opus | Deep architectural analysis |
| Implementation | workflow:executor | sonnet | Careful implementation |
| Code Review | workflow:reviewer-deep | opus | Comprehensive review |
| Security | workflow:security-deep | opus | Deep security audit |
| Tests | workflow:test-writer | sonnet | Full test coverage |
| Performance | workflow:perf-reviewer | sonnet | Performance analysis |
| Documentation | workflow:doc-writer | haiku | Documentation updates |

*NEVER use built-in agents without =workflow:= prefix - they may use bash for file ops*
